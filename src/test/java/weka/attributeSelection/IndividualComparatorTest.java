package weka.attributeSelection;

import java.util.HashMap;
import java.util.Iterator;
import java.util.Random;

import junit.framework.TestCase;

public class IndividualComparatorTest extends TestCase {

	public IndividualComparatorTest(String name) {
		super(name);
	}
	
	protected void setUp() throws Exception {
		super.setUp();
	}
	
	protected void tearDown() throws Exception {
		super.tearDown();
	}
	
	public void testExecute() throws EvolutionaryException {
		int populationSize, sizeOfIndividuals, classIndex;
		double fitness;
		boolean hasClass;
		Population population;
		Random random;
		Operator randomInit;
		HashMap<String, Object> parameters;
		Iterator<Individual> iterator;
		IndividualComparator ic;
		
		populationSize = 10;
		population = new Population(populationSize);
		sizeOfIndividuals = 10;
		classIndex = sizeOfIndividuals - 1;
		hasClass = true;
		random = new Random();
		parameters = new HashMap<String, Object>();
		parameters.put("random", random);
		parameters.put("populationSize", populationSize);
		parameters.put("classIndex", classIndex);
		parameters.put("sizeOfIndividuals", sizeOfIndividuals);
		parameters.put("hasClass", hasClass);
		
		randomInit = InitializationFactory.getInitializationOperator("RandomInitialization", parameters); 
		
		randomInit.execute(population);
		System.out.println(population);
		
		iterator = population.getIterator();
		fitness = 10.0;
		
//		while(iterator.hasNext()) {
//			iterator.next().setFitness(random.nextDouble());
//			iterator.next().setFitness(fitness);
//			fitness -= 0.1;
//		}
		
		population.getIndividual(0).setFitness(0.0);
		population.getIndividual(1).setFitness(0.0);
		population.getIndividual(2).setFitness(0.1);
		population.getIndividual(3).setFitness(0.1);
		population.getIndividual(4).setFitness(0.2);
		population.getIndividual(5).setFitness(0.2);
		population.getIndividual(6).setFitness(0.5);
		population.getIndividual(7).setFitness(0.5);
		population.getIndividual(8).setFitness(0.8);
		population.getIndividual(9).setFitness(0.8);
		
		System.out.println(population);
		ic = new IndividualComparator();
		population.sort(ic);
		
		System.out.println(population);
		
	}

}
